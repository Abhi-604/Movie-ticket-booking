...
void print_basic_screen()
{
 ...
 {
     FILE *fp;
     fp=fopen("e:\\Movie Ticket Booking\\Questions.bin","rb");
     if(fp==NULL)
     {
 ...
 {
     FILE *fp;
     fp=fopen("e:\\Movie Ticket Booking\\Users.bin","rb");
     if(fp==NULL)
     {
         ...
         {
             print_char(' ',79,25,BLACK);
             print_title("Error in creating the Users.bin\n",1,25,RED);
             perror("Reason:");
             getch();
             exit(1);
         }
 ...
 int is_valid_integer(char *str)
 {
     while(*str)  //*str!='\0'
     {
         if(isdigit(*str)==0)//isdigit() is not defined yet and not declared yet
            return 0;
         str++;
     }
     return 1;
 ...
 char *get_mobile_number(int row,int col)
 {
     ...
     do
     {
        print_title("Enter your mobile number: +91-",col,row,BROWN);
        ...
        if(is_valid_integer(mobile_number)==0||strlen(mobile_number)!=10)
        {
            ...
            print_title_in_middle("Error! Mobile Number Is Incorrect, Please Re-enter the mobile_number",25,RED);
            getch();
            print_char(' ',80,25,BLACK);
...
user * get_credentials(char * usertype)
{
    ...
    if(strcmp(usertype,"master_admin")==0)
        print_title_in_middle("Enter master user name & password to setup your profile",10,LIGHTCYAN);
   ...
   do
   {
       ...
       if(strcmp(u.userid,"0")==0)
        ...
        {
            print_char(' ',80,25,BLACK);
            print_title_in_middle("Error! Username cannot be blank. Please Try Again",25,RED);
            getch();
        }
        else
        {
...
int check_user_details(user *u)
{
    ...
    while(fread(&usr,sizeof(usr),1,fp)==1)
    {
        if(strcmp(u->usertype,"master_admin")==0)
        {
            ...
            print_title_in_middle("contact to admin if you encounter any trouble during logging",20,YELLOW);
...
void get_integer(int *p)
{
    ...
    *p=atoi(temp);//atoi converts ascii or char to int
}
...
void create_user(char *usertype)
{
    ...
    if(strcmp(usertype,"admin")==0){
        ...
        if(up!=NULL)
        {
            ...
            if(up==NULL){
                print_char(' ',80,25,BLACK);
                print_title_in_middle("Canceling The Admin Profile Setup",25,RED);
                fclose(fp);
                getch();
                exit(0);
            }
        ...
        sprintf(temp,"USER-%d",last_id);
         ...
         up=get_user_details("user");
            if(up==NULL){
                print_char(' ',80,25,BLACK);
                print_title_in_middle("Cencelling The User Profile Setup",25,RED);
                fclose(fp);
                getch();
                exit(0);
            }
...
void show_theater()
{
    ...
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("No Theater Present In The File ",25,RED);
        fclose(fp);
        getch();
        return;
    }
...
char *get_theater_id(int status)
{
    ...
    do{
        ...
        get_integer(&id);//this function is also not defined
        if(id==0)
        {
            fclose(fp);
            return NULL;
...
{
    print_basic_screen();
    FILE *fp=fopen("e:\\Movie Ticket Booking\\Bookings.bin","wb");
    if(fp==NULL)
    {
        print_char(' ',79,25,BLACK);
    ...
    if(ftell(fp)==0){
            fclose(fp);
            remove("e:\\Movie Ticket Booking\\Bookings.bin");
            return 1;
    }

    FILE *fp1=fopen("e:\\Movie Ticket Booking\\Bookings.bin","wb");
    if(fp1==NULL)
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Bookings.bin\n",1,25,RED);
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file TempBookings.bin",1,25,RED);
        perror("Reason:");
        getch();
        fclose(fp1);
        return 0;
...
int delete_movie_by_theater_id(char *th_id)
{
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file TempMovies.bin",1,25,RED);
        perror("Reason:");
        getch();
        fclose(fp1);
        return 0;
...
int delete_theater()
{
    ...
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Cencelling Delete Theater",25,RED);
        getch();
        return 0;
    }
...
void add_movie()
{
    ...
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Cencelling Add Movie",25,RED);
        getch();
        return;
    }
    print_basic_screen();
    ...
    do
    {
        ...
        scanf("%d/%d/%d",&mov.show_date.tm_mday,&mov.show_date.tm_mon,&mov.show_date.tm_year);//28/7/2022  don't 28/07/2022
    ...
    print_title(temp,column,++row,YELLOW);
    sprintf(temp,"Final Date:(%02d/%02d/%04d)",mov.show_date.tm_mday,mov.show_date.tm_mon,mov.show_date.tm_year);//formating of the output
    print_title(temp,column,++row,YELLOW);
    getch();
}

char *get_theater_name(char *theater_id)
{
    FILE *fp=fopen("e:\\Movie Ticket Booking\\theaters.bin","rb");
    if(fp==NULL)
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening file theaters.bin",1,25,RED);
        perror("reason:");
        getch();
        return NULL;
    }
    static char theater_name[200];
    theater th;
    while(fread(&th,sizeof(th),1,fp)==1)
    {
        if(!strcmp(th.theater_id,theater_id)){
            strcpy(theater_name,th.theater_name);
            break;
        }
    }
    fclose(fp);
...
void change_contact_details(user *u)
{
    ...
    FILE *fp;
     ...
     if(fp==NULL)
     {
         ...
         char choice;
         print_title_in_middle("Are you sure to change the contact details?(y/Y/n/N)",9,YELLOW);
         fflush(stdin);
         scanf("%c",&choice);
...
int delete_movie()
{
    char *movie_id=get_movie_id();
    if(movie_id==NULL)
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Cencelling Delete Movie",25,RED);
        getch();
        return 0;
    }
    FILE *fp1=fopen("e:\\Movie Ticket Booking\\Movies.bin","rb");
    ...
    while(fread(&mov,sizeof(mov),1,fp1)==1)
    {
        ...
        {
            resp=delete_booking_by_movie_id(mov.movie_id);
            if(resp==0)
            {
                fclose(fp1);
                fclose(fp2);
...
char *get_movie_id()
{
    ...
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("No Movies Present In The File ",25,RED);
        fclose(fp);
        getch();
        return NULL;
    }
...
void change_password(user *u)
{
    ...
    char choice;
    print_title_in_middle("Are you sure to change the password?(y/Y/n/N)",9,YELLOW);
    fflush(stdin);
    scanf("%c",&choice);
    if(choice=='n'||choice=='N')
    {
    ...
    else if(choice=='Y'||choice=='y')
    {
        ...
        rewind(fp);
        while(fread(&us,(long long signed int)sizeof(user),1,fp)==1)
            {
                if(strcmp(us.userid,u->userid)==0)
                {
                    fseek(fp,-(long long signed int)sizeof(user),SEEK_CUR);
            ...
            getch();
                ...
                print_char(' ',79,25,BLACK);
                print_title_in_middle("Invlaid Choice",25,RED);
                getch();
        }
    fclose(fp);
}
movie *get_movie_id_and_show_date()
{
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title("Movie Details Not Present",1,25,RED);
        fclose(fp);
        getch();
        return NULL;
    }
    ...
    while(fread(&mov,sizeof(mov),1,fp)==1)
    {
        if(strcmp(mov.movie_id,prev_mov_id)!=0)
        {
            ...
            //can we direct print the first block_price without using sprintf
            sprintf(temp_str,"%d",mov.first_block_price);
            print_title(temp_str,25,row,YELLOW);
            sprintf(temp_str,"%d",mov.second_block_price);
            print_title(temp_str,44,row,YELLOW);
    ...
    do
    {
        ...
        if(valid==1)
        {
            ...
            valid=0;
            //verify the show date entered by user is valid or not
            do
            {
...
int *get_seat_numbers(movie *m)
{
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file SeatNumbers.bin\n",1,25,RED);
        perror("Reason:");
        getch();
        return NULL;
    }
    ...
    do
    {
        ...
        do
        {
            ...
            if(seat_number==0)
            {
                    ...
                    {
                        print_char(' ',79,25,BLACK);
                        print_title("Error in Removing SeatNumber.bin!",1,25,RED);
                        perror("Error:");
                        getch();
                ...
                {
                    print_char(' ',79,25,BLACK);
                    print_title_in_middle("Invalid Set Number",25,RED);
                    getch();
                    print_char(' ',79,23,BLACK);
                }
                else
...
void change_seat_booking_status(int seat_number,char *mov_id,struct tm date)
{
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Movies.bin booking(User*u)\n",1,25,RED);
        perror("Error:");
        getch();
        return ;
    }
...
void booking(user *u)
{
    ...
    if(m==NULL)
    {
     ...
     for(int i=0;seat_numbers[i]!=-1;i++)
     {
         if(seat_numbers[i]>=1&&seat_numbers[i]<=30)
            fb_count++;
         if(seat_numbers[i]>=31&&seat_numbers[i]<=70)
            sb_count++;
         if(seat_numbers[i]>=71&&seat_numbers[i]<=100)
            tb_count++;
     }
     print_basic_screen();
    ...
    print_basic_screen();
    fseek(fp,-sizeof(tk)*rec_count,SEEK_END);
    row=8;
...
char* get_ticket_id(user* u)
{
    ...
    if(fp==NULL)
    {
        ...
        perror("Reason:");
        getch();
        return NULL;//check from other functions
    }
    fseek(fp,0,SEEK_END);
    if(ftell(fp)==0)
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("No Bookings Present In The File ",25,RED);
        fclose(fp);
        getch();
        return NULL;
    }
    ...
    int row=8;
    print_title("Ticket ID",1,row,YELLOW);
    print_title("Theater ID",14,row,YELLOW);
    print_title("Movie ID",27,row,YELLOW);
    print_title("Show Date",40,row,YELLOW);
    print_title("Seat Number",55,row,YELLOW);
    ...
    while(fread(&tk,sizeof(tk),1,fp)==1)
    {
        ...
        {
            print_title(tk.ticket_id,1,row,YELLOW);
            print_title(tk.theater_id,14,row,YELLOW);
            print_title(tk.movie_id,27,row,YELLOW);
    ...
    do{
        ...
        if(id==-1)
        {
            ...
            while(fread(&tk,sizeof(tk),1,fp)==1)
            {
                if(strcmp(tk.ticket_id,tk_id)==0)
                {
                    valid=1;
                    break;
                }
...
int cancel_booking_of_user(user *u)
{
    char *ticket_id=get_ticket_id(u);
    if(ticket_id==NULL)
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("Canceling cancel booking process",25,RED);//message is wrong
        getch();
        return 0;
    }
    ...
    if(fp1==NULL){
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Bookings.bin",1,25,RED);
        perror("Reason:");
        getch();
        return 0;
    }
    ...
    while(fread(&tk,sizeof(tk),1,fp1)==1)
    {
        if(strcmp(tk.ticket_id,ticket_id)!=0)
        {
            fwrite(&tk,sizeof(tk),1,fp2);
        }
        else{
    ...
    if(x==0)
    {
        ...
        if (y==0){
            ...
            perror("reason for not renaming file:");
    }
    else
        perror("reason for not removing file:");
    return 0;
}

void show_booking_of_user_(user *u)
{
    print_basic_screen();
    FILE *fp=fopen("e:\\Movie Ticket Booking\\Bookings.bin","rb");
    if(fp==NULL)
    ...
    {
        print_char(' ',80,25,BLACK);
        print_title_in_middle("No Bookings Present",25,RED);
        fclose(fp);
        getch();
        return;
    }
...
void change_security_question(user *u)
{
    ...
    FILE *fp;
     ...
     if(fp==NULL)
     {
         ...
         char choice;
         print_title_in_middle("Are you sure to change the security question?(y/Y/n/N)",9,YELLOW);
         fflush(stdin);
         scanf("%c",&choice);
         if(choice=='n'||choice=='N')
         {
...
int delete_my_account(user *u)
{
    print_basic_screen();
    char choice;
         print_title_in_middle("Are you sure to delete your account?(y/Y/n/N)",9,YELLOW);
         fflush(stdin);
         scanf("%c",&choice);
         if(choice=='n'||choice=='N')
         {
             print_char(' ',79,25,BLACK);
             print_title_in_middle("Canceling delete my account",25,BLUE);
             getch();
         }
         else if(choice=='Y'||choice=='y')
         {
    FILE *fp1=fopen("e:\\Movie Ticket Booking\\Users.bin","rb");
    if(fp1==NULL){
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Users.bin\n",1,25,RED);
        perror("Reason:");
    ...
    FILE *fp2=fopen("e:\\Movie Ticket Booking\\TempUsers.bin","wb");
    if(fp2==NULL){
        print_char(' ',79,25,BLACK);
    ...
    fclose(fp2);
    int x,y;
    x=remove("e:\\Movie Ticket Booking\\Users.bin");
    if(x==0)
    {
        y=rename("e:\\Movie Ticket Booking\\TempUsers.bin","e:\\Movie Ticket Booking\\Users.bin");
        if (y==0)
            return 1;
        else
            perror("reason for not renaming file:");
...
void forget_credentials(char *usertype)
{
    ...
    do
    {
     ...
     char *p;
        ...
        while(fread(&usr,sizeof(usr),1,fp)==1)
        {
            if(strcmp(usr.userid,temp)==0&&strcmp(usr.mobile_number,p)==0)
            {
                flag=1;
                print_basic_screen();
                p=get_question_id();
                ...
                if(strcmp(usr.question_id,p)==0&&strcmp(usr.hint_answer,temp)==0)
                {
                    print_basic_screen();
                    input_password(&usr,25,12);
                    if(strcmp(usr.password,"0")==0)
                        break;
                    fseek(fp,-(long long signed int)sizeof(user),SEEK_CUR);
